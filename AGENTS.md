...
- Android TV agent lives in `apps/android-tv/` (see its README). JVM protocol tests/demo need JDK 17+ only: `cd apps/android-tv && ./gradlew :core:test :core:runAgentDemo` with `cloud:dev` running (demo bootstraps dashboard Bearer token). APK builds need `ANDROID_HOME` / `local.properties` `sdk.dir` (see `local.properties.example`). Optional release signing via `keystore.properties` / `BULWARK_TV_STORE_*` — without secrets, `:app:assembleRelease` stays unsigned. Emulator → host control plane URL is `http://10.0.2.2:8787`. Remote isolate / DNS policy requires on-device `VpnService.prepare()` — until approved, command results use `applied: false` + `reason=vpn_permission_required`, inventory reports `dnsGuard.vpnConsentPending`, and the parent panel shows **VPN PENDING** plus an on-TV approve hint. Never auto-launch the VPN dialog from WorkManager; denial keeps pending and offers Retry Approve. Clearing isolation must not clear pending if `dnsGuardRequired` remains true.
