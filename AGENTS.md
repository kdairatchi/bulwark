...
- Phase 1 safety: prefer `gatedCleanItems` for file cleans (IPC/cloud/CLI) so `requireRestorePoint` cannot be bypassed. Destructive malware delete / debloat remove / registry fix also call `assertCleanAllowed` and honor `{ dryRun, force }` (CLI `--dry-run` / `--force`). Linux/mac → `platform_unsupported` (allowed). Quarantine accepts `parameters.dryRun`. Activity & Logs UX polish remains deferred — see `ROADMAP.md`.
