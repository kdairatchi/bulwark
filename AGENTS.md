...
- Phase 1 safety: `cleanItems(..., origin, { dryRun: true })` enumerates would-delete without removing files; CLI `--dry-run` with `--clean` does the same. Prefer `gatedCleanItems` from IPC/cloud/CLI so `settings.cleaner.requireRestorePoint` cannot be bypassed (scheduled cleans inherit the gate). Linux/mac → `platform_unsupported` (allowed). Quarantine accepts `parameters.dryRun`. Activity & Logs UX polish remains deferred — see `ROADMAP.md`.
- Phase 1 safety: `cleanItems(..., origin, { dryRun: true })` enumerates would-delete without removing files; CLI `--dry-run` with `--clean` does the same. `assertDestructiveAllowed` (`destructive-action-gate.ts`) blocks cleans when `settings.cleaner.requireRestorePoint` is on and Windows restore-point creation fails (Linux/mac → `platform_unsupported`, allowed). Quarantine accepts `parameters.dryRun`. Activity & Logs UX polish remains deferred — see `ROADMAP.md`.
