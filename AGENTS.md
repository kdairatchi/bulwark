...
Bulwrk is a single Electron desktop app (React 19 renderer + Node main process, built by electron-vite). There is no separate backend/frontend server. Commands (`npm run dev`, `npm test`, `npm run validate:rules`, `npm run build`) are documented in `CLAUDE.md` / `CONTRIBUTING.md`. Product name is **Bulwrk**; public marketing domain and support email stay open (cloud hosts via `BULWRK_CLOUD_URL`). Internal bridge/metrics may still say `kudu` until a dedicated migration.

- Dependencies are refreshed by the startup update script (`npm ci`); its `postinstall` runs `patch-package` and `electron-rebuild -w better-sqlite3`. If native modules misbehave after changing Electron/Node, re-run `npm ci` to rebuild `better-sqlite3`.
