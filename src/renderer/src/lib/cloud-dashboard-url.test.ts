...
describe('publicCloudDashboardUrl', () => {
  const prev = process.env.BULWARK_CLOUD_URL

  afterEach(() => {
    if (prev === undefined) delete process.env.BULWARK_CLOUD_URL
    else process.env.BULWARK_CLOUD_URL = prev
    vi.unstubAllEnvs()
  })

  beforeEach(() => {
    delete process.env.BULWARK_CLOUD_URL
  })

  it('returns null when unset or placeholder', () => {
    expect(publicCloudDashboardUrl()).toBeNull()
    process.env.BULWARK_CLOUD_URL = 'https://cloud.invalid'
    expect(publicCloudDashboardUrl()).toBeNull()
    process.env.BULWARK_CLOUD_URL = 'https://cloud.usekudu.com'
    expect(publicCloudDashboardUrl()).toBeNull()
  })

  it('returns configured https URL with optional path', () => {
    process.env.BULWARK_CLOUD_URL = 'https://example-cloud.test/'
