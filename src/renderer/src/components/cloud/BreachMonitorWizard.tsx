...
import { explainBreachMonitorSetup } from '@/lib/breach-monitor-explain'

/**
 * Guided first-add wizard for parent email breach monitors.
 */
export function BreachMonitorWizard({
  open,
  busy,
  email,
  usage,
  ...
  onConfirmAdd: () => void | Promise<void> | boolean | Promise<boolean>
}) {
  const [step, setStep] = useState(0)
  const total = 4
  const explained = explainBreachMonitorSetup()
  const accent = '#60a5fa'
  ...
  const titles = [
    t('breachWizardStepMeaningTitle'),
    t('breachWizardStepPrivacyTitle'),
    t('breachWizardStepConfirmTitle'),
    t('breachWizardStepDoneTitle'),
  ]
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={t('breachWizardTitle')}
    >
      <motion.div
      ...
      >
        <button
          ...
          aria-label={t('breachWizardClose')}
        >
          <X className="h-4 w-4" />
        </button>
        ...
        <div className="mb-5 flex items-center gap-3 pr-8">
          ...
          >
            <Mail className="h-5 w-5" style={{ color: accent }} strokeWidth={1.8} />
          </div>
          <div>
            <h3 className="text-[16px] font-semibold text-white">{t('breachWizardTitle')}</h3>
            <p className="text-[12px]" style={{ color: 'var(--text-muted)' }}>
              {t('breachWizardSubtitle', { usage, limit })}
            </p>
          </div>
        </div>
          ...
          >
            ...
            {step === 0 && (
              <FindingExplainPanel
                ...
                accent={accent}
                whyTitle={t('breachWizardWhy')}
                recommendedTitle={t('breachWizardRecommended')}
                animate={false}
              />
            )}
            ...
            {step === 1 && (
              <div className="space-y-4 text-[12px]">
                <ImpactList
                  icon={ShieldAlert}
                  title={t('breachWizardWhatHappens')}
                  items={explained.whatHappens}
                  color={accent}
                />
                <ImpactList
                  icon={Lock}
                  title={t('breachWizardPrivacy')}
                  items={explained.privacy}
                  color="#a78bfa"
                />
              </div>
            )}
            ...
            {step === 2 && (
              <div className="space-y-3">
                ...
                <div
                  ...
                  style={{ background: 'rgba(96,165,250,0.08)', border: '1px solid rgba(96,165,250,0.28)', color: '#bfdbfe' }}
                >
                  <div className="flex items-start gap-2">
                    <ShieldAlert className="h-4 w-4 shrink-0 mt-0.5" strokeWidth={1.8} />
                    <span>{t('breachWizardConfirmHint')}</span>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="flex gap-2">
            {step > 0 && step < total - 1 && (
              <GhostButton onClick={() => setStep((s) => s - 1)} disabled={busy}>
                <ChevronLeft className="h-3.5 w-3.5" />{t('breachWizardBack')}
              </GhostButton>
            )}
            {step === 0 && (
              <GhostButton onClick={onClose} disabled={busy}>{t('breachWizardCancel')}</GhostButton>
            )}
          </div>
          ...
          <div className="flex gap-2">
            {step < 2 && (
              <PrimaryButton accent={accent} onClick={() => setStep((s) => s + 1)}>
                {t('breachWizardContinue')} <ChevronRight className="h-3.5 w-3.5" />
              </PrimaryButton>
            )}
            {step === 2 && (
              <PrimaryButton
                ...
                onClick={async () => {
                  const result = await onConfirmAdd()
                  if (result === false) return
                  setStep(total - 1)
                }}
              >
                {busy ? t('breachWizardWorking') : t('breachWizardConfirmAction')}
              </PrimaryButton>
            )}
            {step === total - 1 && (
