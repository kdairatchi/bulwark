...
const api = {
  ...
  systemClean: (itemIds: string[], options?: { dryRun?: boolean }): Promise<CleanResult> =>
    ipcRenderer.invoke(IPC.SYSTEM_CLEAN, itemIds, options),

  // Browser cleaner
  browserScan: (): Promise<ScanResult[]> => ipcRenderer.invoke(IPC.BROWSER_SCAN),
  browserClean: (itemIds: string[], options?: { dryRun?: boolean }): Promise<CleanResult> =>
    ipcRenderer.invoke(IPC.BROWSER_CLEAN, itemIds, options),

  // App cleaner
  appScan: (): Promise<ScanResult[]> => ipcRenderer.invoke(IPC.APP_SCAN),
  appClean: (itemIds: string[], options?: { dryRun?: boolean }): Promise<CleanResult> =>
    ipcRenderer.invoke(IPC.APP_CLEAN, itemIds, options),

  // Gaming cleaner
  gamingScan: (): Promise<ScanResult[]> => ipcRenderer.invoke(IPC.GAMING_SCAN),
  gamingClean: (itemIds: string[], options?: { dryRun?: boolean }): Promise<CleanResult> =>
    ipcRenderer.invoke(IPC.GAMING_CLEAN, itemIds, options),
  ...
  uninstallLeftoversClean: (itemIds: string[], options?: { dryRun?: boolean }): Promise<CleanResult> =>
    ipcRenderer.invoke(IPC.UNINSTALL_LEFTOVERS_CLEAN, itemIds, options),

  // Recycle bin
  recycleBinScan: (): Promise<ScanResult[]> => ipcRenderer.invoke(IPC.RECYCLE_BIN_SCAN),
  recycleBinClean: (options?: { dryRun?: boolean }): Promise<CleanResult> =>
    ipcRenderer.invoke(IPC.RECYCLE_BIN_CLEAN, options),

  // Shortcut cleaner
  shortcutScan: (): Promise<ScanResult[]> => ipcRenderer.invoke(IPC.SHORTCUT_SCAN),
  shortcutClean: (itemIds: string[], options?: { dryRun?: boolean }): Promise<CleanResult> =>
    ipcRenderer.invoke(IPC.SHORTCUT_CLEAN, itemIds, options),
